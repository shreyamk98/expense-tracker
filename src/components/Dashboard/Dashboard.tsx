...
export const Dashboard: React.FC<DashboardProps> = ({
	...
	const categoryData = Object.values(ExpenseCategory)
		.map((category) => {
	...
	const paymentData = Object.values(PaymentType)
		.map((type) => {
						...
						<div>
							...
							<Text size="xl" fw={600}>
								{recentExpenses.length > 0
									? formatCurrency(recentExpenses[0].amount)
									: formatCurrency(0)}
							</Text>
							<Text size="xs" c="dimmed">
								{recentExpenses.length > 0
									? formatDate(new Date(recentExpenses[0].date))
									: 'No expenses'}
							</Text>
						</div>
						...
						{budgetProgress.length > 0 ? (
							budgetProgress.map((budget) => (
								<div key={budget.id}>
									...
									<Progress
										...
										color={
											budget.isOverBudget ? 'red' : budget.percentage > 80 ? 'yellow' : 'green'
										}
										size="sm"
									/>
								</div>
							...
							<Stack gap="sm" align="center" py="md">
								...
								<Text size="xs" ta="center" c="dimmed">
									Create budgets to track your spending limits and get insights on your financial
									goals
								</Text>
							</Stack>
						)}
