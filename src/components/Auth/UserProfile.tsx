							...
							{isEditing ? (
								<Group grow>
									...
									<DateInput
										...
										onChange={(date) =>
											setFormData((prev) => ({
												...prev,
												dateOfBirth: date ? new Date(date).toISOString().split('T')[0] : '',
											}))
										}
								...
								<Group>
									...
									{authState.currentUser.dateOfBirth && (
										<Text size="sm" c="dimmed">
											🎂 {new Date(authState.currentUser.dateOfBirth).toLocaleDateString()}
										</Text>
									)}
								</Group>
							)}
					...
					{isEditing ? (
								...
								<Select
									...
									onChange={(value) =>
										setFormData((prev) => ({
											...
											preferences: {
												...prev.preferences!,
												defaultBudgetPeriod: (value as TimePeriod) || TimePeriod.MONTH,
											},
										}))
									}
								/>
								...
								onChange={(value) =>
									setFormData((prev) => ({
										...
										preferences: {
											...prev.preferences!,
											notifications:
												(value as NotificationPreference) || NotificationPreference.IMPORTANT,
										},
									}))
								}
							/>
						...
						<Stack gap="xs">
							...
							<Group justify="space-between">
								...
								<Text size="sm" fw={500}>
									{authState.currentUser.preferences.defaultBudgetPeriod}
								</Text>
							</Group>
							<Group justify="space-between">
								<Text size="sm">Notifications</Text>
								<Text size="sm" fw={500}>
									{formatNotificationPreference(authState.currentUser.preferences.notifications)}
								</Text>
							</Group>
							<Group justify="space-between">
